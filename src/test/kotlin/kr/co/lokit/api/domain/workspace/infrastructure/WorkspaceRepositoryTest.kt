...
class WorkspaceRepositoryTest {
    ...
    @BeforeEach
    fun setUp() {
        user = userJpaRepository.save(UserEntity(email = "test@test.com", name = "테스트", role = UserRole.USER))
    }
    ...
    fun `초대 코드로 워크스페이스를 조회할 수 있다`() {
        val saved = workspaceRepository.saveWithUser(Workspace(name = "팀"), user.id)
    ...
    fun `워크스페이스에 유저를 추가할 수 있다`() {
        val saved = workspaceRepository.saveWithUser(Workspace(name = "팀"), user.id)
        val user2 = userJpaRepository.save(UserEntity(email = "user2@test.com", name = "유저2", role = UserRole.USER))
