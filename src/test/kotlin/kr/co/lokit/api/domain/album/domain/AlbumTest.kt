...
class AlbumTest {
    ...
    fun `정상적인 제목으로 앨범을 생성할 수 있다`() {
        val album = Album(title = "여행 앨범", coupleId = 1L)

        assertEquals("여행 앨범", album.title)
        assertEquals(1L, album.coupleId)
    ...
    fun `10자 이하의 제목으로 앨범을 생성할 수 있다`() {
        val album = Album(title = "1234567890", coupleId = 1L)
    ...
    fun `빈 제목으로 앨범을 생성할 수 있다`() {
        val album = Album(title = "", coupleId = 1L)
    ...
    fun `10자 초과 제목으로 앨범을 생성하면 예외가 발생한다`() {
        val exception = assertThrows<IllegalArgumentException> {
            Album(title = "12345678901", coupleId = 1L)
        }
