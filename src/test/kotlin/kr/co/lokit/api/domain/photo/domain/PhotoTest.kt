...
class PhotoTest {
    ...
    @Test
    fun `기본값이 올바르게 설정된다`() {
        val photo = Photo(albumId = 1L, location = Location(0.0, 0.0))

        assertEquals(0L, photo.id)
        assertNull(photo.description)
        assertNull(photo.url)
        assertNull(photo.uploadedById)
        assertNull(photo.takenAt)
    }

    @Test
    fun `url과 uploadedById를 설정할 수 있다`() {
        val photo = Photo(albumId = 1L, location = Location(127.0, 37.5))
        photo.url = "https://example.com/photo.jpg"
        photo.uploadedById = 1L
