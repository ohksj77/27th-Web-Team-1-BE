...
class AlbumRepositoryTest {
    ...
    fun setUp() {
        ...
        val coupleUser = CoupleUserEntity(couple = couple, user = user)
        couple.addUser(coupleUser)
        couple = coupleJpaRepository.save(couple)
    }

    @Test
    fun `앨범을 저장할 수 있다`() {
        val album = createAlbum(title = "여행", coupleId = couple.nonNullId())

        val saved = albumRepository.save(album, user.nonNullId())

        assertNotNull(saved.id)
        assertEquals("여행", saved.title)
        assertEquals(couple.id, saved.coupleId)
    }
    ...
    fun `photos가 비어있는 앨범도 findAllByUserId로 조회된다`() {
        val album = AlbumEntity(title = "빈 앨범", couple = couple, createdBy = user)
    ...
    fun `photos가 있는 앨범과 없는 앨범이 함께 조회된다`() {
        val albumWithPhotos = AlbumEntity(title = "사진 앨범", couple = couple, createdBy = user)
        ...
        val emptyAlbum = AlbumEntity(title = "빈 앨범", couple = couple, createdBy = user)
